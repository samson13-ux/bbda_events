...
| `image\` | `images\` |
| `bbda_docs\README.md`, `bbda_docs\AI_RULES.md` | déplacés à la racine du projet |
...
- 1 admin, 2 agents, 4 organisateurs (statuts de compte variés : actif,
  ...
  bcrypt (jamais en clair, conformément à `AI_RULES.md` §5).
...
`requirements.txt`, donc interdit par `AI_RULES.md` §6) : les formulaires
...
- **Pas de Flask-WTF/CSRF** : la bibliothèque n'est pas dans
  `requirements.txt` et `AI_RULES.md` interdit d'utiliser une dépendance non
...
- **`base.html`** : navigation simplifiée en liens texte (Accueil /
  ...
  premier fichier JS du projet, vanilla comme l'exige `AI_RULES.md` §3) ;
...
- **`frontend/static/js/main.js`** : logique vanilla JS (RM impose
  l'absence de framework JS, AI_RULES.md §3) pour (1) afficher la section
...
- **`backend/statuts.py`** (nouveau) : extraction de `STATUTS_AFFICHAGE`
  ...
  blueprints `declarations` et `agent` (AI_RULES.md §8, pas de code
  duplique).
...
- **`frontend/static/js/main.js`** : `initialiserTotalMontant()` recalcule
  ...
  (JavaScript vanilla, conforme a `AI_RULES.md`).
